...
class show_officers {
    ...
    SwingWorker<Boolean, Void> worker = new SwingWorker<Boolean, Void>() {
      ...
      protected Boolean doInBackground() {
        ...
        try {
          ...
          Logger.getLogger(show_officers.class.getName()).log(Level.SEVERE, null, ae);
        }

        try {
          con = DriverManager.getConnection("jdbc:mysql://localhost:3306/test","root","muskan");

          String sql = "select * from police_officer where PO_PS=?";
          ...
          while (rs.next()) {
            model.addRow(new Object[] { rs.getString(1), rs.getString(2), rs.getString(5), rs.getString(8),
                rs.getString(4), rs.getString(7), rs.getString(6) });
          }
          ...
          Logger.getLogger(show_officers.class.getName()).log(Level.SEVERE, null, ee);
        } finally {
          try {
            con.close();
          } catch (SQLException ee) {
      ...
      protected void done() {
        ...
        try {
          status = get();
          if (status == true) {
            frame = new JFrame("VIEW ALL OFFICERS");
            mainpanel = new JPanel(new BorderLayout(10, 10));
            panel = new JPanel(new GridLayout(1, 3, 0, 0));
            lblCase = new JLabel("<HTML><h1>Police Officers</h1></HTML>", JLabel.CENTER);
          ...
          Logger.getLogger(show_officers.class.getName()).log(Level.SEVERE, null, e);
  ...
  public static void init() {
    ...
    show_officers obj = new show_officers();
    obj.display();
  }
