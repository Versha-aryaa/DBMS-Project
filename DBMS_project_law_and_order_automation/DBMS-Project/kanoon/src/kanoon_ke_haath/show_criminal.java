...
class show_criminal {
  ...
  public void display() {
    SwingWorker<Boolean, Void> worker = new SwingWorker<Boolean, Void>() {
      ...
      protected Boolean doInBackground() {
        ...
        model.addColumn("<HTML><h3>Age</h3></HTML>");
        model.addColumn("<HTML><h3>Gender</h3></HTML>");
        model.addColumn("<HTML><h3>Contact</h3></HTML>");
        model.addColumn("<HTML><h3>Crime</h3></HTML>");
        model.addColumn("<HTML><h3>Date of crime</h3></HTML>");
        try {
          Class.forName("com.mysql.jdbc.cj.Driver");
        } catch (ClassNotFoundException ae) {
          Logger.getLogger(show_criminal.class.getName()).log(Level.SEVERE, null, ae);
        }

        try {
          con = DriverManager.getConnection("jdbc:mysql://localhost:3306/test","root","muskan");

          String sql = "select * from criminal where CRIM_PS=?";
          ...
          Logger.getLogger(show_criminal.class.getName()).log(Level.SEVERE, null, ee);
        } finally {
          try {
            con.close();
          } catch (SQLException ee) {
      ...
      protected void done() {
        ...
        try {
          status = get();
          if (status == true) {
            frame = new JFrame("VIEW ALL CRIMINALS");
            mainpanel = new JPanel(new BorderLayout(10, 10));
            panel = new JPanel(new GridLayout(1, 3, 0, 0));
            lblCase = new JLabel("<HTML><h1>Criminals</h1></HTML>", JLabel.CENTER);
          ...
          Logger.getLogger(show_criminal.class.getName()).log(Level.SEVERE, null, e);
  ...
  public static void init() {
    ...
    show_criminal obj = new show_criminal();
    obj.display();
  }
